...
/**
 * Helper to register for and send broadcasts of Intents to local objects within your process. This
 * is has a number of advantages over sending global broadcasts with {@link
 * Context#sendBroadcast}:
 *
 * <ul>
 *   <li>You know that the data you are broadcasting won't leave your app, so don't need to worry
 *       about leaking private data.
 *   <li>It is not possible for other applications to send these broadcasts to your app, so you
...
class LocalBroadcastManager {
  ...
  /**
   * Unregister a previously registered BroadcastReceiver. <em>All</em> filters that have been
  ...
  /**
   * Like {@link #sendBroadcast(Intent)}, but if there are any receivers for the Intent this
   * function will block and immediately dispatch them before returning.
   */
  ...
  private void executePendingBroadcasts() {
    while (true) {
      ...
      synchronized (mReceivers) {
        final int totalPendingBroadcasts = mPendingBroadcasts.size();
        if (totalPendingBroadcasts <= 0) {
          return;
        }
        brs = new BroadcastRecord[totalPendingBroadcasts];
